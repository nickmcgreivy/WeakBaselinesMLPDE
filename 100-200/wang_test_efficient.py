...
def _godunov_flux_1D_burgers_pad(u_left, u_right):
    zero_out = 0.5 * jnp.abs(jnp.sign(u_left) + jnp.sign(u_right))
    compare = jnp.less(u_left, u_right)
    F = lambda u: u**2 / 2
    FL = F(u_left)
    FR = F(u_right)
    return compare * zero_out * jnp.minimum(FL, FR) + (
        1 - compare
    ) * jnp.maximum(FL, FR)

def time_derivative_1D_burgers_pad(a, t, nx, dx, nu):
    a = jnp.pad(a, (1, 1), 'wrap')
    uL = a[:-1] 
    uR = a[1:]
    flux = _godunov_flux_1D_burgers_pad(uL, uR)

    flux = flux - (nu / dx) * (uR - uL)
...
def simulate_1D(a0, t0, nx, dx, dt, nt, nu, output=False):
    dadt = lambda a, t: time_derivative_1D_burgers_pad(a, t, nx, dx, nu)
